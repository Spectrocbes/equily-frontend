...
## 2026-08-21 — feat/broker-logos-country-flags (complete)
- **Country flags** in the three geographic sections (investment detail exposure, analytics geo allocation, rebalancing targets + suggestions). Deviated from the plan on evidence: importing `flag-icons`' stylesheet emits all 542 SVGs it ships — **3.9 MB, 77% of a 5.0 MB build**, measured — to render 8. Instead it is a devDependency and `npm run sync:flags` copies only the codes declared in `country-flags.ts` (25 flags, 106 KB). The script also deletes flags no longer declared, so removing a country shrinks the build
- **Broker logos** on the four wealth lists, the detail headers, and the add/delete account modals. Favicons fetched once and committed, not hot-linked — the deciding argument is privacy: a live request per logo would tell the icon provider which institutions each user banks with, on every page view. Also makes the UI deterministic and drops a runtime dependency on an undocumented endpoint
- **Filenames declared, never derived.** A `slugify()` breaks on `De Giro` → `degiro.png` and `Caisse d'Épargne` → `caisse-depargne.png`. Lookups are case-insensitive because accounts store `BoursoBank` while the catalogue lists `Boursobank`
- **Two shared components** (`app-region-flag`, `app-broker-logo`) rather than markup repeated per call site — 14 templates touch this data and the brief caps duplication at 3%. Both degrade gracefully (neutral mark / initials) so a missing asset never leaves a broken-image glyph, and both are `aria-hidden` since the name is always rendered beside them
- **Framing is per brand, centring is automatic.** Centring is derivable (`normalize-broker-logos.ps1`); how tightly a mark should crop is not — the alpha bounding box reports full-bleed logos as perfectly centred because it measures the canvas, not the mark. `LOGO_FRAMING` in `broker-logos.ts` carries an optional scale + x/y per file, applied as a CSS transform: non-destructive, legible, and safe from an asset refresh. **Manual framing beats auto-centring** — the two compounded on LCL until the script learned to skip hand-framed files
- **Resolution capped at 192px** (44px frame × 4x density × max zoom). Better sources took the assets to 547 KB, one logo at 2048px/252 KB; capping returns 209 KB with no visible change. Same argument as the flags
- **Anti-drift specs**: every declared flag and logo file must exist on disk, every catalogue broker must resolve to a logo or a deliberate fallback, and no framing entry may point at a missing file. Verified the disk check actually fails by removing `fr.svg`. Framing specs derive their expectations from the manifest rather than pinning literals, which would otherwise need editing after every visual nudge
- **Mistake worth recording:** `git checkout` on `lcl.png` destroyed an uncommitted higher-resolution replacement. Recovered via the SVG, which is a better asset than the one lost — but the loss was avoidable. Check `git status` for uncommitted replacements before restoring any file
- Remaining: 10 logos were low-resolution favicons; better sources have since replaced most. `bourse-direct` has no usable favicon and renders as initials by design
- 699/699 tests, coverage 84.4% stmt / 85.9% lines, lint clean, build 0 errors
